...
function render(contacts) {
  ...
  const contactsHTMLArray = contacts.map((contact) => {
...
const indexOfContactToDelete = window.contacts.findIndex(contact => contact.id === Number(id));
...
function deleteButtonHandler() {
  ...
  allDeleteButtons.forEach((deleteBtn) => {
    deleteBtn.addEventListener("click", (event) => {
      const idOfPersonToDelete = event.target.parentNode.getAttribute("data-id");
